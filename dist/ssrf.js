...
const defaultResolver = async (hostname) => {
    ...
    return results.map((r) => ({ ip: r.address, family: r.family }));
};
 ...
 */
export function classifyIp(ipStr) {
    let addr;
    try {
    ...
    if (addr.kind() === "ipv6") {
        ...
        const r = v6.range();
        if (BLOCKED_V6_RANGES.has(r))
            return `ipv6:${r}`;
        return null;
    }
    const r = addr.range();
    if (BLOCKED_V4_RANGES.has(r))
        return `ipv4:${r}`;
    return null;
}
 ...
 */
export async function validateUrl(rawUrl, resolver = defaultResolver) {
    let url;
    try {
        url = new URL(rawUrl);
    }
