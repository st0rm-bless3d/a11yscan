...
export function tripsCopyGuard(text) {
    return BANNED.some((re) => re.test(text));
}
const SYSTEM_PROMPT = "You are an accessibility engineer helping a developer fix a specific WCAG issue " +
...
async function callLlm(rule, resolver) {
 ...
 * is unset.
 */
export async function fixGuidanceFor(rules, resolver) {
    const config = getConfig();
    const out = new Map();
    for (const r of rules)
        out.set(r.id, { text: r.help, source: "axe" });
    if (!config.llm.baseUrl)
        return out; // --fix-hints with no endpoint configured
    if (rules.length > config.llm.maxRules)
        return out; // too many; skip LLM entirely
    const cache = new Map();
    const queue = [...rules];
...
//# sourceMappingURL=fixhints.js.map