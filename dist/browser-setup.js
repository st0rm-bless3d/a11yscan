...
export function classifyLaunchFailure(message) {
    ...
    if (HOST_DEPS_PATTERNS.some((re) => re.test(msg)))
        return "missing-host-deps";
    if (MISSING_BROWSER_PATTERNS.some((re) => re.test(msg)))
...
export function resolvePlaywrightCli() {
    ...
    if (!pkgDir)
        return null;
    try {
        const pkg = JSON.parse(readFileSync(resolvePath(pkgDir, "package.json"), "utf8"));
 ...
 */
export function installChromium(cliPath, timeoutMs = 600_000) {
    ...
    if (res.signal)
        return { ok: false, reason: `terminated by signal ${res.signal}` };
    if (res.status !== 0)
        return { ok: false, reason: `exited with code ${res.status}` };
    return { ok: true };
}
...
export function playwrightCommand(args) {
    ...
    if (cli)
        return `node ${quotePath(cli)} ${args.join(" ")}`;
 ...
 */
export function browserSetupGuidance(kind, ctx) {
    const version = playwrightVersion();
    ...
    if (kind === "missing-browser") {
        ...
        lines.push("Install it with the Playwright that ships with a11yscan, so the browser revision matches:", `    ${playwrightCommand(["install", "chromium"])}`, "That path points at a11yscan's own Playwright, so the revision matches and it runs from any directory.");
        return lines.join("\n");
    }
    const lines = ["a11yscan could not start a browser."];
    if (ctx.upstream)
        lines.push(ctx.upstream);
    lines.push("If this looks like a browser-installation problem, install Chromium with a11yscan's own Playwright:", `    ${playwrightCommand(["install", "--with-deps", "chromium"])}`);
    return lines.join("\n");
}
