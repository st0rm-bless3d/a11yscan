...
const ACTIONABLE =
  /(playwright\S*\s+install|install-deps|apt-get\s+install|npm\s+(?:i|install)\b|npx\s+)/i;
 ...
 */
export function sanitizeErrorText(raw: string, opts: SanitizeOptions = {}): string {
  ...
  for (let i = 0; i < lines.length && keep.size < ceiling; i++) {
    if (ACTIONABLE.test(lines[i] as string)) keep.add(i);
  }
  const headBudget = Math.max(1, maxLines - keep.size - 1);
  for (let i = 0; i < lines.length && i < headBudget; i++) keep.add(i);
