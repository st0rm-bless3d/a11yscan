 ...
 */
export async function pinnedFetch(targetUrl: string, opts: PinnedFetchOptions): Promise<PinnedResponse> {
  ...
  for (const [k, v] of Object.entries(opts.headers)) {
    if (!STRIP_REQUEST_HEADERS.has(k.toLowerCase())) reqHeaders[k] = v;
  }
  ...
  try {
    ...
    if (err instanceof PinnedFetchError) throw err;
    throw new PinnedFetchError("fetch-failed", (err as Error).message ?? "Fetch failed.");
  } finally {
